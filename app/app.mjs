...
document.addEventListener('keydown',e=>{if(document.querySelector('dialog[open]')||e.target.matches('input,textarea,select')||e.metaKey||e.ctrlKey||e.altKey)return;if(e.repeat)return;if(e.key==='Enter'&&e.target.closest('button,a'))return;if(/^[1-8]$/.test(e.key)){e.preventDefault();selectKey(Number(e.key)-1)}else if(e.key==='ArrowLeft'){e.preventDefault();rotate(-1)}else if(e.key==='ArrowRight'){e.preventDefault();rotate(1)}else if(e.key==='Enter'){e.preventDefault();confirm()}else if(e.key==='Escape'){cancelGame();choice=MODES[mode].id==='dress'?state.outfit:0;render();speak('不着急，我们慢慢选。')}});
let batteryTimer;
const handleKeyboardEvent=e=>{if(e.type==='battery'){const el=$('#battery-level');el.hidden=false;el.textContent=e.percent<0?'电量未知':`电量 ${e.percent}%`;el.title='键盘电量';clearTimeout(batteryTimer);batteryTimer=setTimeout(()=>{el.textContent='电量待更新'},25000);return}const dialog=document.querySelector('dialog[open]');if(e.type==='long_press'){if(window.momoDesktop?.focusWindow)void window.momoDesktop.focusWindow().catch(()=>toast('暂时无法唤回 Momo 窗口，请点击 Dock 中的 Momo 重试。'));if(dialog)dialog.close();else{cancelGame();choice=MODES[mode].id==='dress'?state.outfit:0;render();speak('回来啦，我们换一件事做吧。')}return}if(e.type==='key'&&e.key===5&&(!dialog||dialog.id==='chat-dialog')){selectKey(4);return}if(dialog){if(dialog.id==='chat-dialog'&&(e.type==='key'||e.type==='rotate'))dialog.close();else return}if(e.type==='key')selectKey(e.key-1);if(e.type==='rotate')rotate(e.delta*(reverse?-1:1));if(e.type==='press')confirm()};
...
const voice=new VoiceChat({getProfile:()=>petProfile.voice==='default'&&petProfile.persona==='default'?null:{...petProfile},createAudio:options=>{
  ...
  onState:(status,text)=>{
    ...
    $('#chat-input').disabled=active&&!['listening','ready'].includes(status);$('#send-chat').disabled=chatBusy||(active&&!['listening','ready'].includes(status));
...
async function pressS5(){
  ...
  if(voice.active&&!voice.session.manual){toast('请先关闭连续语音，再按 S5 使用按键留言');return}
  if(voice.active&&!['listening','ready'].includes(voice.state)){toast('正在准备或回复，请稍等后再按 S5');return}
  music.stop();render();voiceReply=null;await voice.pressToTalk();
}
