...
export function parseEvent(line) {
  ...
  if(e.type==='hello' && e.board==='easyinput-v2' && typeof e.firmware==='string') return {type:'hello',firmware:e.firmware.slice(0,32),sound:e.sound===true,mic:e.mic==='pcm16-usb-v1',...(e.wireless===true&&/^[0-9a-f]{12}$/.test(e.device)?{wirelessDevice:e.device}:{})};
  if(e.type==='wireless_state'&&Number.isInteger(e.id)&&e.id>0&&e.id<=2147483647&&typeof e.ok==='boolean'&&/^[0-9a-f]{12}$/.test(e.device)&&typeof e.error==='string'&&e.error.length<=40)return {type:e.type,id:e.id,ok:e.ok,device:e.device,error:e.error};
  ...
  if(e.type==='mic_state'&&Number.isInteger(e.id)&&e.id>=0&&e.id<=2147483647&&Number.isInteger(e.stream)&&e.stream>0&&e.stream<=2147483647&&typeof e.ok==='boolean'&&typeof e.active==='boolean'&&e.rate===16000&&typeof e.error==='string'&&e.error.length<=40)return {type:e.type,id:e.id,stream:e.stream,ok:e.ok,active:e.active,error:e.error,rate:e.rate};
  if(e.type==='mic_audio'&&Number.isInteger(e.stream)&&e.stream>0&&e.stream<=2147483647&&Number.isInteger(e.seq)&&e.seq>=0&&e.seq<=4294967295&&typeof e.pcm==='string'&&/^[A-Za-z0-9+/]{342}==$/.test(e.pcm))return {type:e.type,stream:e.stream,seq:e.seq,pcm:e.pcm};
