...
export class VoiceChat {
  ...
  async start(){
    ...
    const s={queue:[],playing:false,turn:false,synth:false,end:false,ack:false,reply:'',audio:null};this.session=s;
    ...
    try{
      ...
      s.audio=this.createAudio({onSpeech:samples=>this.submitAudio(samples),onLimit:()=>this.stop('这一段说得太久了，请重新打开麦克风，分段说。'),onError:message=>{if(this.session===s)this.stop(message)}});
  ...
  async submitAudio(samples){
    const s=this.session;if(!s||this.state!=='listening'||samples.length<6400)return false;
    if(samples.length>16000*60||!samples.every(n=>Number.isFinite(n)&&Math.abs(n)<=1)){this.stop('录音太长或格式不正确，请分段说。');return false}
    this.beginTurn(s);
    try{await s.audio.pause();if(this.session!==s)return false;
    ...
    }catch{if(this.session===s)this.stop('录音发送失败，请重试。');return false}
  }
  async submitText(text){
    const s=this.session;if(!s||this.state!=='listening'||!text.trim())return false;
    this.beginTurn(s);
    try{await s.audio.pause();if(this.session!==s)return false;this.onText(text);this.send(s,{type:'text-input',text});return true}
    catch{if(this.session===s)this.stop('消息发送失败，请重试。');return false}
  }
  async drain(s){
  ...
  finish(s){
    ...
    if(s.end&&!s.resuming){s.resuming=true;void (async()=>{
      ...
      catch{if(this.session===s)this.stop('麦克风恢复失败，请重试。')}
    })()}
  }
  stop(message){
