...
3. 选择聊天活动（S5 或界面的“聊天”）。此时只选择活动，不会自动录音。
4. 旋转旋钮选择“语音聊天”，短按旋钮；用鼠标也可点击“开始语音聊天”。
5. 首次使用允许 Momo / Electron 访问麦克风。出现“正在听”后说一句话。
6. 说完停顿约一秒，等待“正在想／正在说”；恢复“正在听”后说下一句。

没连接键盘也可以操作界面。桌面键盘的数字 5 选择聊天，左右箭头切换选项，Enter 确认（输入框获得焦点时保持正常打字）。
...
4. 按 S5，旋钮转到“语音聊天”，按下旋钮。
5. 看到“正在听”后对键盘说话；回答从电脑播放。

BLE 蓝牙连接只传控制和电量，不传麦克风音频；`USB Serial` 桥接串口 `/dev/cu.usbserial…` 也不提供本版麦克风能力。没有能力握手时 Momo 会提示升级固件或改用原生 USB，不会偷偷切回电脑录音。

本次 0.5.0 已写入实板且数据校验通过，但重启后的新固件运行与真人板载收音仍待验证。不要将这个状态理解成已经通过所有硬件验收；完整记录见 [键盘语音输入说明](KEYBOARD-VOICE-INPUT.md)。
...
| 旋钮选项 | 按下后 |
| --- | --- |
| 语音聊天 | 打开聊天窗口并启用保存的默认麦克风；已有会话时返回原会话 |
| 文字聊天 | 打开聊天窗口，不自动打开麦克风 |
| 关闭麦克风 | 停止语音采集与播放，睡眠状态下也能执行 |
