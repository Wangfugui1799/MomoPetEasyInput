...
2026-09-12 / 桌面 0.6.3：按用户新要求，S5 改为「按一下录音、再按发送」，回复后待命；聊天活动中短按旋钮只显示对话框。下文旧的两步开始方式已被替代，当前操作与验收见 [S5 按键留言](S5-VOICE-MESSAGE.md)。固件未改，无需烧录。

2026-09-11 更新：用户明确确认 USB 连续真人三轮通过。下面 2026-09-09 的未验证条目为当时记录，不覆盖这一后续用户实测反馈；其他电声、异常断线与共存项目仍须分别验收。桌面 0.6.1 保留 USB，并修复 Wi-Fi 短暂积压误断开与回合间停止确认，固件仍为 0.6.0。
