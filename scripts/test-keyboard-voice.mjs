...
const manual=process.env.MOMO_VOICE_MANUAL==='1';
...
try{
  ...
  await page.evaluate(async base64=>{
    ...
    const ctx=new AudioContext({sampleRate:16000}),buffer=await ctx.decodeAudioData(Uint8Array.from(atob(base64),c=>c.charCodeAt(0)).buffer);const samples=buffer.getChannelData(0);await ctx.close();window.fixtureFrames=Math.ceil(samples.length/128);
  ...
  if(manual)await page.evaluate(()=>{window.usbEvent({type:'key',key:5});window.usbEvent({type:'press'})});
  else {await page.locator('#chat-open').click();await page.locator('#voice-toggle').click()}
  for(let turn=1;turn<=3;turn++){
    ...
    if(manual){
      const before=await page.evaluate(()=>window.usbPCMFrames);
      await page.waitForFunction(before=>window.usbPCMFrames-before>=window.fixtureFrames,before,{timeout:60000});
      await page.evaluate(()=>window.usbEvent({type:'key',key:5}));
    }
    ...
    await page.waitForFunction(manual=>[manual?'ready':'listening','error'].includes(document.querySelector('#voice-status').dataset.state),manual,{timeout:90000});
    if(await page.locator('#voice-status').getAttribute('data-state')==='error')throw Error(await page.locator('#voice-status').textContent());
    console.log((manual?'S5 manual capture':'Real VAD')+' / real backend with simulated USB: completed turn '+turn);
    if(manual&&turn<3)await page.evaluate(()=>window.usbEvent({type:'key',key:5}));
  }
