...
output="$(mktemp -d "${TMPDIR:-/tmp}/momo-mic-command-test.XXXXXX")"
trap 'rm -rf "$output"' EXIT
cc -std=c11 -Wall -Wextra -Werror -I"$project_dir/firmware/tests/stubs" -I"$sdk_dir/components/json/cJSON" \
  "$project_dir/firmware/main/microphone.c" "$project_dir/firmware/main/microphone_logic.c" "$project_dir/firmware/tests/microphone_commands_test.c" \
