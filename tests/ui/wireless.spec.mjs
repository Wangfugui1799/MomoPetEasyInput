...
async function setup(page,{connected=true,bound=true}={}){
  await page.addInitScript(({connected,bound})=>{
    ...
    window.momoDesktop={setAlwaysOnTop:async()=>{},onWireless:fn=>{window.listeners.push(fn);return ()=>{}},wireless:async(action,value)=>{
      ...
      if(action==='mic'){
        if(value.type==='mic_start'){stream=value.stream;seq=0;timer=setInterval(()=>window.wirelessEmit('mic',{type:'mic_audio',stream,seq:seq++,pcm:btoa('\0'.repeat(256))}),8)}
        if(value.type==='mic_stop')clearInterval(timer);
...
test('wireless source completes three turns with no USB or system microphone',async({page})=>{
  await setup(page);await page.keyboard.press('5');await page.keyboard.press('Enter');await expect(page.locator('#voice-status')).toContainText('正在听');
  for(let i=0;i<3;i++){
